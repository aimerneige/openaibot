...
import (
	...
	"github.com/aimerneige/openaibot/internal/pkg/utils"
)
...
func SendQueryRequest(apiReq ApiReq, accessToken, signToken, aeskey string) string {
	...
	reqJson := string(data)
	cipher, err := utils.Encrypt(aeskey, reqJson)
	if err != nil {
		panic(err)
	}
...
func apiCall(accessToken, signToken, aeskey string, body []byte) string {
	...
	nonce := utils.GenRandomLetterString(QueryNonceLength)
	sign := utils.CalcSign(signToken, timestamp, nonce, body)
	...
	plain, err := utils.Decrypt(aeskey, cipher)
	if err != nil {
		panic(err)
	}
	apiResp := ApiRsp{}
